...
function timestamp() {
	# EPOCHREALTIME is available in bash 5+; if unavailable, consider using: date +%s.%N
...
check_mandatory_param "${ACTION:-}" "Please provide a valid value for action."
check_mandatory_param "${DEV_PATH:-}" "Please provide a valid value for device path."

if [[ "${ACTION}" == "add" ]]; then
	check_mandatory_param "${MAJOR_VERSION:-}" "Please provide a valid value for major number."
	check_mandatory_param "${MINOR_VERSION:-}" "Please provide a valid value for minor number."
	sudo mknod -m 0666 "${DEV_PATH}" c "${MAJOR_VERSION}" "${MINOR_VERSION}"
	...
	# Remove the device node and its parent if provided.
	sudo rm -f "${DEV_PATH}" "${PARENT_PATH}"
